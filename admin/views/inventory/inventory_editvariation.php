<div id="content">
	...
	<div id="section-channels" class="section section-closed">

		<div class="table-row">
			<h3>Sales Channels</h3>
			<p>Select the channels this product should be included in.</p>
		</div>
		...
		<div class="table-row">
			...
			foreach($locations as $channel) {
				...
				if ($channel->id == 1 && count($locations) == 1) {
			?>
				<label for="<?=$channel_field;?>"><input type="checkbox" name="fake_checkbox" value="1" checked="checked" disabled="disabled" /> <?=$channel->name;?></label>
				<input type="hidden" name="<?=$channel_field;?>" value="1" readonly="readonly" />
				<?php
				} else {
				?>
				<label for="<?=$channel_field;?>"><input type="checkbox" name="<?=$channel_field;?>" id="<?=$channel_field;?>" value="1" <?=is_checked('1', set_value($channel_field, $channel_field_value));?> /> <?=$channel->name;?></label>
			<?php 
				}
			} 
			?>
...
<div id="sidebar">
	...
	<ul id="sections">
		...
		<li><a href="#section-channels" class="button button-off">Sales channels</a></li>
	</ul>
</div>
</div>